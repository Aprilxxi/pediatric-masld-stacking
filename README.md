...
Consequently, the exact numerical results cannot be reproduced from this repository alone. Authorized users must place the required development and external-validation datasets in a local working directory and update the local file paths in the scripts.
...
5. For external validation, each saved fold-specific model generated a probability for every external participant. The current external-validation script uses the patient-level median of the ten probabilities as the final score.
...
| LightGBM | `feature_fraction=0.6`, `learning_rate=0.005`, `max_depth=5`, effective `num_leaves=6`, `subsample=0.8`, `n_estimators=800`, `min_child_weight=1`, `lambda_l1=5`, `lambda_l2=200` |
...
In the original Python source, LightGBM used `num_leaves=2 ^ 4`. Because `^` is the bitwise XOR operator in Python, its effective value is 6; this is reported explicitly above to preserve the executed analysis.
...
Then place the restricted datasets in a local directory, update the paths and filenames in the scripts, and run the required analysis. The main script is a research script rather than a packaged command-line application; the order of columns in the supplied datasets must match that used during model development.
