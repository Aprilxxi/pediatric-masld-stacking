...
from bootstrap_ci_utils import (
...
for train_index, test_index in kf.split(y):
    if j == 9:
        ...
        explainer = shap.KernelExplainer(current_clf.predict_proba, background)

        shap_values = explainer.shap_values(X_test)

        expected = explainer.expected_value

        proba_model = current_clf.predict_proba(X_test)
        ...
        if isinstance(shap_values, list):
            values_pos = np.array(shap_values[pos_idx])
            expected_pos = expected[pos_idx] if np.ndim(expected) > 0 else expected
        else:
            values_array = np.array(shap_values)
            if values_array.ndim == 3:
                values_pos = values_array[:, :, pos_idx]
            else:
                values_pos = values_array
            expected_pos = expected[pos_idx] if np.ndim(expected) > 0 else expected
        base_pos = np.full(X_test.shape[0], expected_pos)
        ...
        shap.plots.waterfall(shap_exp[5], max_display=20)

        fig = plt.figure()
        shap.plots.beeswarm(
            shap_exp, max_display=11, show=False
        )
        plt.title("AdaBoostClassifier SHAP values", loc="center")
        plt.savefig(
            "AdaBoostClassifier SHAP values.pdf",
            dpi=300,
            bbox_inches="tight",
            pad_inches=0,
        )
...
for train_index, test_index in kf.split(y):
    if j == 1:
        ...
        explainer = shap.Explainer(current_clf)
        shap_values = explainer(X_test)
        fig = plt.figure()
        shap.plots.beeswarm(
            shap_values, max_display=21, order=shap_values.abs.max(0), show=False
        )
        plt.title("LGBM SHAP values", loc="center")
        plt.savefig("LGBM SHAP values.png", dpi=300, bbox_inches="tight", pad_inches=0)
    j += 1
...
for train_index, test_index in kf.split(y):
    if j == 1:
        ...
        explainer = shap.LinearExplainer(current_clf, masker=masker)
        shap_values = explainer.shap_values(X_test)
        shap_exp = shap.Explanation(np.array(shap_values), data=X_test)
        fig = plt.figure()
        shap.plots.beeswarm(
            shap_exp, max_display=21, order=shap_exp.abs.max(0), show=False
        )
        plt.title("LogisticRegression SHAP values", loc="center")
        plt.savefig(
            "LogisticRegression SHAP values.png",
            dpi=300,
            bbox_inches="tight",
            pad_inches=0,
        )
...
for train_index, test_index in kf.split(y):
    if j == 1:
        ...
        plt.title("XGB SHAP values", loc="center")
        plt.savefig("XGB SHAP values.png", dpi=300, bbox_inches="tight", pad_inches=0)
    j += 1
# %%
model_name = "LGBMClassifier"
j = 0
...
for train_index, test_index in kf.split(y):
    if j == 1:
        ...
        shap_values = explainer.shap_values(X_test)
        shap_exp = shap.Explanation(np.array(shap_values[:, :, 1]), data=X_test)
        fig = plt.figure()
        shap.plots.beeswarm(
            shap_exp, max_display=21, order=shap_exp.abs.max(0), show=False
        )
        plt.title("RandomForest SHAP values", loc="center")
        plt.savefig(
            "RandomForest SHAP values.png", dpi=300, bbox_inches="tight", pad_inches=0
        )
    j += 1
# %%
...
for train_index, test_index in kf.split(y):
    if j == 1:
        ...
        plt.title("ExtraTree SHAP values", loc="center")
        plt.savefig(
            "ExtraTree SHAP values.png", dpi=300, bbox_inches="tight", pad_inches=0
        )
    j += 1
# %%
model_name = "AdaBoostClassifier"
...
for train_index, test_index in kf.split(y):
    if j == 1:
        ...
        logit_base = np.array(sv.base_values)
        shap_vals = np.array(sv.values)
        if logit_base.ndim == 0:
            logit_base = np.full(len(X_test), logit_base)
        if logit_base.ndim == 1:
            logit_total = logit_base + shap_vals.sum(axis=1)
            proba_pos = expit(logit_total)
            proba_recon = np.column_stack([1.0 - proba_pos, proba_pos])
            values_pos = shap_vals
            base_pos = proba_recon[:, 1]
        else:
            if shap_vals.ndim == 2:
                shap_vals = shap_vals[:, :, np.newaxis]
            logit_total = logit_base + shap_vals.sum(axis=1)
            proba_recon = softmax(logit_total, axis=1)
            pos_idx = int(np.where(current_clf.classes_ == 1)[0][0])
            values_pos = shap_vals[:, :, pos_idx]
            base_pos = proba_recon[:, pos_idx]

        proba_model = current_clf.predict_proba(X_test)
        if proba_recon.shape[1] == 2:
            logits = np.log(proba_recon[:, 1] / proba_recon[:, 0])
            decision_model = current_clf.decision_function(X_test)
            A = np.column_stack([np.ones_like(logits), logits])
            coeffs, _, _, _ = np.linalg.lstsq(A, decision_model, rcond=None)
            corrected_logit = coeffs[0] + coeffs[1] * logits
            adj_pos = expit(corrected_logit)
            proba_recon = np.column_stack([1.0 - adj_pos, adj_pos])
            values_pos = shap_vals
            base_pos = proba_recon[:, 1]
        if not np.allclose(proba_recon, proba_model, atol=1e-9, rtol=1e-7):
            diff = np.max(np.abs(proba_recon - proba_model))
            print('[WARN] Reconstructed proba differs from model by up to {:.3e}'.format(diff))
        ...
        plt.title("AdaBoostClassifier SHAP values", loc="center")
        plt.savefig(
            "AdaBoostClassifier SHAP values.png",
            dpi=300,
            bbox_inches="tight",
            pad_inches=0,
        )
...
for train_index, test_index in kf.split(y):
    if j == 1:
        ...
        fig = plt.figure()
        shap.plots.beeswarm(
            shap_exp, max_display=11, order=shap_exp.abs.max(0), show=False
        )
        plt.title("LogisticRegression SHAP values", loc="center")
        plt.savefig(
            "LogisticRegression SHAP values.png",
...
for model_name in model_name_all:
    ...
    for train_index, test_index in kf.split(y):
        if j == 9:
            ...
            title = model_name + " SHAP values"
            plt.title(title, loc="center")
            plt.savefig(title + ".png", dpi=300, bbox_inches="tight", pad_inches=0)
        j += 1
...
# %%
fold_index = np.zeros([len(X),1])
n = 1
for train_index, test_index in kf.split(y):
    fold_index[test_index,0] = n
    n = n+1
...
y_pre_pop = np.concatenate((np.concatenate((y_pre_p[:,1:],y_pred_pcab),axis=1),fold_index),axis=1)
y_pre_pop  = pd.DataFrame(y_pre_pop,columns=index_1)
# %%
with pd.ExcelWriter("模型预测Probability_9.10.xlsx") as writer:
    y_pre_pop.to_excel(writer,  index=False)
